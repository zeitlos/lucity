...
Each service has a name consisting of 2 to 16 characters, lowercase letters, digits and hyphens. That name shows up in the private hostname, the generated domain, and the logs, so pick something you'll still recognise in six months. You cannot change it later.
...
The command runs through `/bin/sh -c`, which means shell syntax works: variable expansion, `&&`, and semicolons all behave as you would expect. The catch is that the shell becomes PID 1 and your process becomes its child, so the shutdown signal at the end of a rollout goes to the shell rather than to your application. Prefix the real process with `exec` so that it replaces the shell and receives the signal itself:

```bash
npx prisma migrate deploy && exec node server.js
```

Without `exec`, your application finds out that it is being shut down when it gets killed. With it, the application gets the full grace period it is owed.
...
- [Deployments](/deployments) for what happens on each release
