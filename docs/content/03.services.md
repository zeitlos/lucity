...
When a [deployment](/deployments) replaces an instance, the old one is asked to stop rather than killed outright. Your process receives `SIGTERM` and has [about twenty seconds](/deployments#how-a-rollout-works) to finish what it is doing and exit on its own. Ignore the signal and it is killed when the time runs out, dropping whatever was still in flight.
