...
Frameworks get their own handling on top of the language. Next.js, Astro, Vite, Angular, React Router and Expo are recognised on the Node side, Django on the Python side, Laravel on the PHP side, each with the build and start commands that framework expects. The [guides](/guides) cover the ones people ship most.
...
Each service keeps its own build cache in the platform registry, next to its images. Every successful build exports its layers there, and every build starts by importing them.

What invalidates what follows the order of the steps:
...
The build log is the whole story, and it streams live in the **Deployments** tab while the build runs. Nothing is deployed when a build fails, so the version that was already running keeps running.

A few failures look stranger than they are:

- **The log stops mid-command with no error.** Almost always memory. Builds run with 4 GB, shared with any other builds happening at the same time, and a large frontend build can exhaust it. Raising the service's memory does not help, since that setting applies to the running container. Cap the toolchain's memory instead, for example `NODE_OPTIONS` set to `--max-old-space-size=3072`.
- **The build is killed after 30 minutes.** That is the limit, and builds are not retried automatically. Deploy again once you have made it faster.
- **A dependency install fails only here.** Lockfiles generated on macOS sometimes omit platform-specific packages that Linux needs. Regenerating the lockfile on Linux fixes it.
...
Detection and the build plan come from [Railpack](https://railpack.com), an open-source builder from the Railway team. The plan is compiled into a [BuildKit](https://github.com/moby/buildkit) build graph and solved by a shared BuildKit daemon, which is what does the layer caching and the parallelism you can see in the log.

What comes out is an ordinary OCI image in the platform's registry, pinned by digest. There is nothing Lucity-specific inside it, so the same image runs under any container runtime, which is part of what makes [ejecting](/eject) work.
