...
Lucity is six services that orchestrate builds and deployments on Kubernetes. No magic, no black boxes. Just well-defined responsibilities and standard protocols. This page explains how the pieces fit together for the curious and the skeptical.
...
| Builder | 9001 | gRPC | Builds container images from source code using [railpack](https://github.com/nicholasgasior/railpack). Pushes to the OCI registry. |
...
| Webhook | 9004 | HTTP | Receives GitHub webhooks. Routes push and PR events to trigger builds and manage preview environments. |
...
| **GitHub App** | Source code access and OAuth authentication | Git provider is an interface |

All open source. All replaceable. The Git provider, build engine, and registry are interfaces in the codebase — swapping implementations is a design goal, not an afterthought.
