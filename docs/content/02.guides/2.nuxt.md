...
Nuxt server routes run in the same container, so a [PostgreSQL database](/postgres) is just a connection away. Link `DATABASE_URL` to the database's `fqdn-uri` reference, then use it from `server/api/`:
