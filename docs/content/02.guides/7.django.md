...
`requirements.txt` matters more than it looks: it is how the [builder](/builds) recognises the project as Python and finds `gunicorn`. Push the result to GitHub and connect it as described in the [quickstart](/quickstart).
...
Add a [PostgreSQL database](/postgres), then link `DATABASE_URL` to its `fqdn-uri` reference and let `dj-database-url` do the parsing:
...
`psycopg[binary]` belongs in your requirements alongside it. Because the value is a [reference](/variables) rather than a pasted string, a rotated password reaches the app without you editing anything.
