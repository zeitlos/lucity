...
Sessions and rate limiting want [Redis](/redis) rather than in-memory state, for the same reason. Anything held in memory belongs to one replica and disappears when it rolls.
...
- [Redis](/redis) for sessions, caching and queues
