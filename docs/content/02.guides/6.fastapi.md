...
Add a [PostgreSQL database](/postgres) and link `DATABASE_URL` to its `fqdn-uri` reference. FastAPI's concurrency model means an async driver is worth having:
...
The reference gives you a `postgresql://` URL, which SQLAlchemy reads as the sync driver, hence the rewrite. Keep the pool small and remember it is per replica.
