...
Give it the same variables as the web service so it reaches the same database and the same [key-value store](/redis), and leave its port unset: it serves no traffic and needs no domain. Scale it by raising replicas on that service alone.

The scheduler works the same way, with `php artisan schedule:work` as its command.

## Next steps

- [Key-value store](/redis) for queues, cache and sessions
