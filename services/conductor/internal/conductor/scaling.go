...
func (c *Client) SetServiceScaling(ctx context.Context, projectID, environment, service string, replicas int, autoscaling *AutoscalingConfig) (*ScalingConfig, error) {
	...
	defer cancel()
	if err := c.Deployer.SetServiceScaling(callCtx, ws, projectID, environment, service, replicas, as); err != nil {
	...
	defer pkgCancel()
	if pkgErr := c.Packager.SetServiceScaling(pkgCtx, ws, projectID, environment, service, replicas, as); pkgErr != nil {
