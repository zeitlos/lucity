...
//
// workspace is taken as an explicit argument rather than read from
// context — this package does not couple to the tenant carrier.
func Build(ctx context.Context, workspace string, repoFiles map[string][]byte, project string) ([]byte, error) {
	ws := workspace
