...
// Reconciler manages Certificate + Gateway listener lifecycle for
// user-declared custom domains. It is a self-contained reconciliation
// loop driven by ReconcileCustomDomains; the Provision/Delete/Status
// methods are direct request handlers for synchronous mutations.
//
// Method signatures still take proto-shaped requests during the
// conductor migration to keep the diff minimal; a follow-up will
// switch them to plain Go arguments once the gRPC surface is gone.
...
// New constructs a Reconciler. The dynamic + typed clients are passed
// in by the caller (constructed from the same kubeconfig).
func New(dyn dynamic.Interface, k8s kubernetes.Interface, gatewayName, gatewayNamespace, clusterIssuer string) *Reconciler {
	return &Reconciler{
		dynamic:          dyn,
		k8s:              k8s,
...
func (s *Reconciler) ProvisionCustomDomain(ctx context.Context, req *deployer.ProvisionCustomDomainRequest) (*deployer.ProvisionCustomDomainResponse, error) {
...
func (s *Reconciler) DeleteCustomDomain(ctx context.Context, req *deployer.DeleteCustomDomainRequest) (*deployer.DeleteCustomDomainResponse, error) {
	hostname := req.GetHostname()
	resourceName := customDomainResourceName(hostname)
	secretName := resourceName + "-tls"
...
func (s *Reconciler) CustomDomainStatus(ctx context.Context, req *deployer.CustomDomainStatusRequest) (*deployer.CustomDomainStatusResponse, error) {
	hostname := req.GetHostname()
	resourceName := customDomainResourceName(hostname)
...
func (s *Reconciler) addGatewayListener(ctx context.Context, hostname, protocol, secretName string) error {
	resourceName := customDomainResourceName(hostname)
...
func (s *Reconciler) removeGatewayListener(ctx context.Context, listenerName string) error {
	gw, err := s.dynamic.Resource(gatewayGVR).Namespace(s.gatewayNamespace).Get(ctx, s.gatewayName, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("failed to get gateway %s: %w", s.gatewayName, err)
	}
...
func (s *Reconciler) ReconcileCustomDomains(ctx context.Context) error {
