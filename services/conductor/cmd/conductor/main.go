...
//
// In phase 3a it wires the same gRPC-client architecture that gateway
// uses today: the handler holds packager/deployer/builder/cashier
// clients and dispatches over the network. Subsequent phases lift
// workspace into handler signatures (3b) and replace the in-cluster
// gRPC clients with in-process calls into deployer.Backend +
// internal/builder Service (3c).
package main
...
type Config struct {
	...
	// Backend services. In phase 3c these collapse — packager + deployer
	// + builder go in-process and these env vars disappear.
