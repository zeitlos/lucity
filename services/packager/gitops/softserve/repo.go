...
// ProjectName derives project name from repo name by stripping workspace prefix and suffix.
// Repo format: {workspace}-{project}-{repoSuffix}
func (r *Repo) ProjectName(ctx context.Context) string {
	wsPrefix := tenant.FromContext(ctx) + "-"
	name := strings.TrimPrefix(r.slug, wsPrefix)
